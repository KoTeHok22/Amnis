...
### 12.5 TelegramUser (задел)
- Модель `TelegramUser` в БД (telegram_id, username, phone_number, access_token, is_verified)
- Эндпоинты `/auth/register/telegram`, `/telegram/auth` присутствуют в `main.py`
- **Активного Telegram-бота нет** — только модель и эндпоинты (вероятно, задел на будущее)
...
| 12 | `auth.py` не читает SECRET_KEY из env-переменных | P0 |
| 13 | Нет регистрации/логирования — только `print()` | P2 |
